 ...
 */
export function updateAllLanguages(entry: FileTypes.ItemEntry, value: string): void {
 ...
 */
export function transformAllLanguages(entry: FileTypes.ItemEntry, transformFn: (text: string) => string): void {
	Object.keys(entry).forEach(key => {
		if (key === 'id' || key === 'Key' || key === 'code' || key === '*eol')
			return;
		if (typeof entry[key] === 'string' && entry[key].trim() !== '')
			entry[key] = transformFn(entry[key]);
	});
}
