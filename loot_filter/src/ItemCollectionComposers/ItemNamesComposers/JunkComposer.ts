...
export class JunkComposer extends ItemCollectionComposerBase implements IItemCollectionComposer {
	...
	protected applyBuffPotions(): void {
		const buffPots: { key: string; name: string; }[] = [
	...
	protected applyThrowingPotions(): void {
		const highlightGas = new SingleHighlight(CharConstants.o, ColorConstants.darkGreen, HighlightConstants.padding.p1);
		const highlightOil = new SingleHighlight(CharConstants.o, ColorConstants.orange, HighlightConstants.padding.p1);

		const throwingPots: { key: string; name: string; highlight: IHighlight; }[] = [
	...
	protected applyAmmo(): void {
		const aqv: string  = 'aqv';
		const cqv: string  = 'cqv';
